...
pub fn write_ntriples(
    ...
    match emit {
        ...
        EmitMode::Closure => {
            for (instance, class) in store.asserted_types() {
                writeln!(
                    writer,
                    "{}",
                    format_type_triple(dictionary, instance, class)?
            ...
            for (subject, predicate, object) in store.asserted_properties() {
                writeln!(
                    writer,
                    "{}",
                    format_property_triple(dictionary, subject, predicate, object)?
...
pub fn write_run_report(path: &Path, report: &report::RunReport) -> Result<()> {
    ...
    writer.write_all(report.to_json_pretty().as_bytes())?;
    writer.flush()?;
    Ok(())
}
...
fn decode_term<'a>(dictionary: &'a Dictionary, identifier: u64, role: &str) -> Result<&'a Term> {
    dictionary.decode(identifier).ok_or_else(|| {
        AppError::new(format!(
            "missing dictionary entry for {role} id {identifier}"
        ))
    })
}
