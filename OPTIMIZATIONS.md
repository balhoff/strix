...
`build_type_index` and `build_property_index` run once at the start of `inner_fixpoint`. Each iteration's deltas are then merged into the persistent indexes via `insert_sorted` before rule application, so join lookups always see known ∪ delta without rebuilding from scratch.

Applied in `27c1aba` (as part of the delta join completeness fix). An earlier attempt in `1ce9eea` used a `dedup_keys` approach that caused a severe performance regression; `insert_sorted` resolved it.

### Engine: reuse buffers in property chain walks

`apply_property_join_rules` now takes a `ChainBuffers` struct with pre-allocated `current`, `next`, and `starts` vectors that are cleared and reused across calls, eliminating per-walk heap allocations.
